<?php
	...
	*/
	...
	/**
	 * @return array All songs list.
	 */
	function hello_dolly_get_all_songs( )
	{
		return array(
			'Hello Dolly',
			'Summertime',
	...
	function hello_dolly_settings_page_render() {
		...
		$all_songs = hello_dolly_get_all_songs();
		...
		<div class='wrap'>
			...
			<form method='post' action="">
				<?php foreach ( $all_songs as $song ) : ?>
					...
					<?php $is_supported = in_array($song, $songs) ?>
					<p><label for="<?php echo $song_id ?>"><input type="radio" id="<?php echo $song_id ?>"
					                                              name="selected_song"
					                                              value="<?php echo $song ?>" <?php checked( $song, $selected_song ) ?> <?php disabled(!$is_supported) ?> /> <?php echo $song . (!$is_supported ? ' <a href="' . hd_fs()->get_upgrade_url() . '">[Upgrade]</a>' : '') ?>
						</label></p>
				<?php endforeach ?>
				<p class='submit'>
