  ...
  return (
    <BlockControls>
      <Toolbar label="Options">
        <ToolbarButton onClick={() => updateLyric(song)} icon={ update } label="Refresh lyric" />
      </Toolbar>
    </BlockControls>
  );
});
...
addFilter('inspector-refresh-lyric', 'hello-dolly/song-lyrics', function (component, song, updateLyric) {
  return (
    <PanelRow>
    <Button class="dashicons-image-rotate" style={{marginTop:'-30px'}} isLink onClick={() => updateLyric(song)}>Refresh song lyric</Button>
  </PanelRow>
  );
});
